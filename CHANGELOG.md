...
- workato-builder subagent dependency (#22): the generation references pointed
  ...
  subagent cannot load. The five format rules (`workato-recipe-format`,
  `workato-agentic-format`, `workato-connector-sdk`, `workato-project-structure`,
  and `workato-page-components` for `lcap_page.json`) are now reachable via
  ...
  single source of truth. The guard derives the format-rule set from rule
  frontmatter, so a new per-asset-type rule is caught automatically. The runtime
  check (can a subagent call MCP tools at all?) is added to the self-install
  smoke's manual checklist for #32 Phase A.
